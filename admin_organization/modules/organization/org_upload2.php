...
if(isset($_FILES['add_file_letter']['name'])){
   $data = array();
   $res_success = 0;
    /* Getting file name */
    $filename = $_FILES['add_file_letter']['name'];
    ...
    if(in_array(strtolower($FileType), $valid_extensions)) {
       /* Upload file */
       if(move_uploaded_file($_FILES['add_file_letter']['tmp_name'],$location.$newname)){
          $response = $newname;
            $res_success = 1;
          $query="
            UPDATE tbl_organization
            SET
            request_letter      = '$response'
