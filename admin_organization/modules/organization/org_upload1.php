...
if(isset($_FILES['add_file_intent']['name'])){
   $data = array();
   $res_success = 0;
    /* Getting file name */
    $filename = $_FILES['add_file_intent']['name'];
    ...
    if(in_array(strtolower($FileType), $valid_extensions)) {
       /* Upload file */
       if(move_uploaded_file($_FILES['add_file_intent']['tmp_name'],$location.$newname)){
          $response = $newname;
        
          $query="
            UPDATE tbl_organization
            SET
            intent_letter         = '$response'
